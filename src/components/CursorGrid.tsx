...
export default function CursorGrid({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const colorArr = Array.isArray(p.color) ? p.color : [p.color ?? "#D946EF"];
      const rgbArr = colorArr.map((c) => typeof c === "string" ? (hexToRgb(c) ?? [100, 100, 100]) : [100, 100, 100]);
      const [gr, gg, gb] = rgbArr.length > 0 ? rgbArr[0] : [100, 100, 100];
      ...
      for (let i = 0; i < alphas.length; i++) {
        ...
        const [cx, cy] = cellCenter(i);
        const ci = (Math.floor(i / cols) + (i % cols)) % rgbArr.length;
        const [cr, cg, cb] = rgbArr[ci];
