...
class App {
  ...
  constructor(container: HTMLElement, { items, bend, textColor, borderRadius, font, scrollSpeed = 2, scrollEase = 0.05 }: any) {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    ...
    const allItems = galleryItems.concat(galleryItems);
    this.medias = allItems.map((data: any, index: number) =>
      new Media({
        geometry: this.planeGeometry, gl: this.gl, image: data.image, index,
        length: allItems.length, renderer: this.renderer, scene: this.scene,
  ...
  addEventListeners() {
    ...
    this.boundOnWheel = (e: WheelEvent) => { this.scroll.target += (e.deltaY > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2; this.onCheckDebounce(); };
    this.boundOnTouchDown = (e: any) => { this.isDown = true; this.position = this.scroll.current; this.start = e.touches ? e.touches[0].clientX : e.clientX; };
    this.boundOnTouchMove = (e: any) => { if (!this.isDown) return; const x = e.touches ? e.touches[0].clientX : e.clientX; this.scroll.target = this.position + (this.start - x) * this.scrollSpeed * 0.025; };
    this.boundOnTouchUp = () => { this.isDown = false; this.onCheck(); };
